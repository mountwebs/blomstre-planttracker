...
const Dashboard = () => {
  ...
  const waterPlant = (id) => {
    const plantIndex = plants.findIndex((el) => el.id === id);
    const nowTime = new Date().getTime();
    let newPlants = [...plants];
    newPlants[plantIndex].watered.push(nowTime);
    setPlants(newPlants);

    const firstWatered = new Date(newPlants[plantIndex].watered[0]);
    console.log(firstWatered.toString());
  };

  const cards = plants.map((plant) => {
    return <MiniCard plant={plant} waterPlant={waterPlant} key={plant.id} />;
  });
  console.log(plants);
